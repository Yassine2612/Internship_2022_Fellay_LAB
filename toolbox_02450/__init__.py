...
def log_r_validate(X,y,cvf=10):
    ''' Validate logistic regression model using 'cvf'-fold cross validation.
        The loss function computed as error rate on validation set .
        Function returns error rate averaged over 'cvf' folds.
    ...
    for train_index, test_index in CV.split(X):
        ...
        validation_error[f] = np.square(y_test-m.predict(X_test).T).sum()/y_test.shape[0]
        
        f=f+1
    return validation_error.mean()      

    
def KNN_validate_fs(X,y,n_neighbors,p,metric,cvf=10):
    ''' Validate KNN regression model using 'cvf'-fold cross validation.
        The loss function computed as error rate validation set.
        Function returns error rate averaged over 'cvf' folds.
        ...
        n_neighbors number of neignbours
        dist used 2 is the euclidean 
        metric=metric,
    ...
    for train_index, test_index in CV.split(X):
        ...
        knclassifier = KNeighborsClassifier(n_neighbors=n_neighbors, p=p, 
        ...
        validation_error[f] = np.sum(y_est_KNN != y_test) / len(y_test)
...
def feature_selector_KNN(X,y,n_neighbors,p,metric ,cvf=10,features_record=None,loss_record=None,display=''):
    ...
    if loss_record is None:
        loss_record = np.array([np.abs(y-y.mean()).sum()/y.shape[0]])
    ...
    for feature in range(0,X.shape[1]):
        if np.where(selected_features==feature)[0].size==0:
            ...
            # validate selected features with KNN classifier and cross-validation:
            trial_loss = KNN_validate_fs(X,y,n_neighbors,p,metric, cvf=10)
    ...
    if best_feature is not False:
        ...
        selected_features, features_record, loss_record = feature_selector_KNN(X,y,n_neighbors,p,metric,cvf,features_record,loss_record)
...
def feature_selector_logr(X,y,cvf=10,features_record=None,loss_record=None,display=''):
    ''' Function performs feature selection for logistic regression model using
        ...
        Example:
        selected_features, features_record, loss_record = ...
            feature_selector_logr(X_train, y_train, cvf=10)
    ...
    for feature in range(0,X.shape[1]):
        if np.where(selected_features==feature)[0].size==0:
            ...
            trial_loss = log_r_validate(X[:,trial_selected],y,cvf)
            if display is 'verbose':
                print(trial_loss)
            if trial_loss<min_loss:
                min_loss = trial_loss 
    ...
    if best_feature is not False:
        ...
        features_record[best_feature,-1]=1
        loss_record = np.concatenate((loss_record,np.array([min_loss])),0)
        selected_features, features_record, loss_record = feature_selector_logr(X,y,cvf,features_record,loss_record)
...
def rlogr_validate(X,y,lambdas,cvf=10):
    ...
    ''' Validate regularized logistic regression model using 'cvf'-fold cross validation.
        Find the optimal lambda (minimizing validation error) from 'lambdas' list.
        The loss function computed as error rate on validation set 
        Function returns: error rate averaged over 'cvf' folds, optimal value of lambda,
        The cross validation splits are standardized based on the mean and standard
        deviation of the training set when estimating the regularization strength.
...
def KNN_validate(X,y,Neighbours,cvf=10):
    
    ''' Validate KNN classifier model using 'cvf'-fold cross validation.
        Find the optimal number of Neighbours (minimizing validation error) from 'Neighbours' list.
        The loss function computed as error rate on validation set 
        Function returns: error rate averaged over 'cvf' folds, optimal number of Neighbours,
        
        
        Parameters:
        X       training data set
        y       vector of values
        Neighbours vector of neignbours values to be validated
        cvf     number of crossvalidation folds     
        
        Returns:
        opt_val_err         validation error for optimum lambda
        opt_neighbours          value of optimal number of neignbours
        train_err_vs_neighbours train error as function of number of neignbours (vector)
        test_err_vs_neighbours  test error as function of number of neignbours (vector)
    '''
    CV = model_selection.KFold(cvf, shuffle=True)
    M = X.shape[1]
    train_error = np.empty((cvf,len(Neighbours)))
    test_error = np.empty((cvf,len(Neighbours)))
    f = 0
    y = y.squeeze()
    for train_index, test_index in CV.split(X,y):
        X_train = X[train_index]
        ...
        for l in range(len(Neighbours)):
            ...
            knclassifier = KNeighborsClassifier(n_neighbors=Neighbours[l]);
            knclassifier.fit(X_train, y_train)
    ...
    opt_val_err = np.min(np.mean(test_error,axis=0))
    opt_neighbours = Neighbours[np.argmin(np.mean(test_error,axis=0))]
    train_err_vs_neighbours = np.mean(train_error,axis=0)
    test_err_vs_neighbours = np.mean(test_error,axis=0)
    
    
    return opt_val_err, opt_neighbours, train_err_vs_neighbours , test_err_vs_neighbours



def ANN_validate(X,y,hidden_units,cvf=10):
    
    ''' Validate ANN CLASSIFICATION model using 'cvf'-fold cross validation.
        Find the optimal number of hidden units  (minimizing validation error) from 'hidden_units' list.
        The loss function computed as error rate on validation set 
        Function returns: error rate averaged over 'cvf' folds, optimal number of hidden units,
        The cross validation splits are standardized based on the mean and standard
        deviation of the training set when estimating the regularization strength.
        ...
        X       training data set
        y       vector of values
        hidden_units vector that contains the different possible number of hidden layer neurons to be validated
        cvf     number of crossvalidation folds     
        
        Returns:
        opt_val_err         validation error for optimum  number of hidden layer neurons
        opt_hidden_units           optimal  number of hidden layer neurons
        
        train_err_vs_hidden_units  train error as function of number of hidden layer neurons(vector)
        test_err_vs_hidden_units  test error as function of number of hidden layer neurons(vector)
    '''
    CV = model_selection.KFold(cvf, shuffle=True)
    ...
    for train_index, test_index in CV.split(X,y):
        ...
        for l in range(len(hidden_units)):
            ...
            model = lambda: torch.nn.Sequential(
                                torch.nn.Linear(M,hidden_units[l] ), #M features to H hiden units
                                torch.nn.Tanh(),   # 1st transfer function,
                                torch.nn.Linear(hidden_units[l], 1), # H hidden units to 1 output neuron
            ...
            y_sigmoid_test = net(X_test)
            y_test_est = (y_sigmoid_test>.5).type(dtype=torch.uint8)
            
            y_sigmoid_train = net(X_train)
            y_train_est = (y_sigmoid_train>.5).type(dtype=torch.uint8)
            
            # Determine test and training errors 
            ...
            train_error[f,l] = (sum(e_train).type(torch.float)/len(y_train)).data.numpy()[0]
            
            e_test = y_test_est != y_test
            
            test_error[f,l] = (sum(e_test).type(torch.float)/len(y_test)).data.numpy()[0]
    ...
    return opt_val_err, opt_hidden_units, test_err_vs_hidden_units ,train_err_vs_hidden_units
